...
class Model
{
	...
	/**
	 * Manages all the associated triggers
	 * 
	 * @var ActiveRecord\TriggerManager
	 */
	private $__trigger_manager = null;
	...
	public function __construct(array $attributes=array(), $guard_attributes=true, $instantiating_via_find=false, $new_record=true)
	{
		...
		// TODO - this will be better if there is only a TriggerManager per Model
		$this->__trigger_manager = new TriggerManager();
		
		if (static::$triggers) {
			
			foreach (static::$triggers as $t) {
				$when = $t[0];
				...
				foreach ($t[1] as $key => $value) {
					...
					$trigger = new Trigger($when, $key, $command);
					$this->__trigger_manager->add_trigger($trigger);
	...
	/**
	 * Add a trigger object to this model
	...
	public function save($validate=true)
	{
		...
		if ($this->is_new_record()) {
			...
			$this->__trigger_manager->execute_before('insert', $this);
			$res = $this->insert($validate);
			$this->__trigger_manager->execute_after('insert', $this);
			
		} else {
			
			$this->__trigger_manager->execute_before('update', $this);
			$res = $this->update($validate);
			$this->__trigger_manager->execute_after('update', $this);
	...
	public function delete()
	{
		...
		$this->__trigger_manager->execute_before('delete', $this);
		static::table()->delete($pk);
		$this->__trigger_manager->execute_after('delete', $this);
